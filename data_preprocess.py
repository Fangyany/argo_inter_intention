...
def train(config):
    ...
    dataset = Dataset("dataset/val_mini/data", config, train=True)
    ...
    for i, data in enumerate(tqdm(test_loader)):
        ...
        for j in range(len(data["idx"])):
            ...
            for key in [
                ...
                'traj1',
                'trajs2'# store['graph']['ctrs'].shape = (1674, 2), num_nodes = 1674, feats.shape = (1674, 2), 
            ]:              # 8 dimention
                store[key] = to_numpy(data[key][j])
                if key in ["graph"]:
                    store[key] = to_int16(store[key])
