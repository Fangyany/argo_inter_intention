...
class ArgoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            data = self.split[idx]
            new_data = dict()
            for key in ['city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph', 'trajs2', 'traj1', 'argo_id', 'label']:
                if key in data:
        ...
        data['idx'] = idx
        data['label'] = self.get_label(data['trajs'][0])
    ...
    def read_argo_data(self, idx):
        ...
        av_ts = np.sort(np.unique(df['TIMESTAMP'].values))
        mapping = dict()
        for i, ts in enumerate(av_ts):
            mapping[ts] = i
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                new_data = dict()
                for key in ['orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph','argo_id','city','traj1', 'trajs2']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
