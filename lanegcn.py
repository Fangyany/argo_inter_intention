...
if "save_dir" not in config:
    config["save_dir"] = os.path.join(
        # root_path, "5000_adame3_conv_adame3_gamma_only20epoch_dis103", model_name
        root_path, "5000_admm_0.01_gamma", model_name
    )
...
"""Dataset"""
# Raw Dataset
config["train_split"] = '/data/fyy/lanegcn/dataset/dataset/train_5000/data'
# config["train_split"] = '/data/fyy/lanegcn/dataset/dataset/train/data'
config["val_split"] = '/data/fyy/lanegcn/dataset/dataset/val/data'
config["test_split"] = '/data/fyy/lanegcn/dataset/dataset/test/data'


# Preprocessed Dataset
config["preprocess"] = True # whether use preprocess or not
config["preprocess_train"] = '/data/fyy/lanegcn/dataset/dataset/preprocess/train_crs_dist6_angle90_5000.p'
# config["preprocess_train"] = '/data/fyy/lanegcn/dataset/dataset/preprocess/train_crs_dist6_angle90.p'
config['preprocess_val'] = '/data/fyy/lanegcn/dataset/dataset/preprocess/val_crs_dist6_angle90.p'
config["preprocess_test"] = '/data/fyy/lanegcn/dataset/dataset/preprocess/test_test.p'
...
class A2A(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(2):
            ...
            # att.append(MultiHeadGAT(config))
        self.att = nn.ModuleList(att)
...
def group_softmax(a, hi):
    group_lengths = torch.bincount(hi)
    groups = torch.split(a, group_lengths.tolist())
    groups = [F.softmax(g, dim=0) for g in groups]
    c = torch.cat(groups)
    return c.view(-1, 1)
...
def get_fake_traj_rel(traj1:List[Tensor], reg:List[Tensor]) -> List[Tensor]:
    ...
    for i in range(config["batch_size"]):
        obs_traj_rel.append(traj1[i][0][:,:2].unsqueeze(0).cuda())
        reg5.append(reg[i][0].unsqueeze(0).cuda())
    obs_traj_rel1 = torch.cat(obs_traj_rel,dim=0)
    obs_traj_rel_repeat6= torch.repeat_interleave(obs_traj_rel1, repeats=6, dim=0)

    reg0 = torch.cat([x for x in reg5], 0).view(-1, 30, 2)

    fake_traj_rel = torch.cat([obs_traj_rel_repeat6, reg0], dim=1) 
        
    return fake_traj_rel.permute(0,2,1)
...
class TrajectoryDiscriminator(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1 = nn.Conv1d(50, 32, kernel_size=2)
    ...
    def forward(self, x):
        ...
        x = nn.functional.max_pool1d(x, kernel_size=1)
        x = self.conv2(x)
        x = nn.functional.relu(x)
        x = nn.functional.max_pool1d(x, kernel_size=1)
        x = x.view(len(x), -1)
...
def softmax_group(a, hi):
    hi_unique = torch.unique(hi)
    c = []
    for h in hi_unique:
        mask = (hi == h)
        group = a[mask]
        group_sum = torch.sum(group)
        group_exp = torch.exp(group)
        group_softmax = group_exp / group_sum
        c.append(group_softmax)
    c = torch.cat(c, dim=0)
    return c
